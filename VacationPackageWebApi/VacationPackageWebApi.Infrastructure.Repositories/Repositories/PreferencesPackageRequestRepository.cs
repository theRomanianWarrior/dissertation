...
public class PreferencesPackageRequestRepository : IPreferencesPackageRequestRepository
{
    ...
    public void SaveEvaluation(ServiceEvaluationDto evaluationOfServices)
    {
        ...
        var directionEvaluation =
            evaluationOfServices.FlightEvaluation.ToEntity(departureFlightEvaluation.Id, returnFlightEvaluation.Id);
        _context.FlightDirectionEvaluations.Add(directionEvaluation);

        foreach (var attractionsEvaluationEntity in
                 evaluationOfServices.AttractionEvaluation.AttractionEvaluations.Select(attractionEvaluations =>
                     attractionEvaluations.ToEntity(allAttractionPoint.Id)))
    ...
    public async Task<Task> UpdateAgentsSelfExpertRate()
    {
        ...
        foreach (var agent in agents)
        {
            ...
            {
                var agentFlightSelfExpertRate =
                    CalculateAgentExpertRateBasedOnTimeRelevance(flightRatings, "Flight", agent.Name, false);
                agent.FlightSelfExpertRate = agentFlightSelfExpertRate;
            }

            if (propertiesRatings != null && propertiesRatings.Any())
            {
                var agentPropertySelfExpertRate =
                    CalculateAgentExpertRateBasedOnTimeRelevance(propertiesRatings, "Property", agent.Name, false);
                agent.PropertySelfExpertRate = agentPropertySelfExpertRate;
            }

            if (attractionsRatings != null && attractionsRatings.Any())
            {
                var agentAttractionSelfExpertRate =
                    CalculateAgentExpertRateBasedOnTimeRelevance(attractionsRatings, "Attractions", agent.Name, false);
    ...
    public async Task UpdateCustomerPersonalAgentRate(ServiceEvaluationDto evaluationOfServices)
    {
        ...
        foreach (var agent in agents)
        {
            ...
            var flightRatingsMatchCustomer =
                await GetAgentFlightRatingsMatchCustomer(agent.Id, customerId, some30DaysAgo);
            var attractionsRatingsMatchCustomer =
                await GetAgentPropertyRatingsMatchCustomer(agent.Id, customerId, some30DaysAgo);
            var propertiesRatingsMatchCustomer =
                await GetAgentAttractionsRatingsMatchCustomer(agent.Id, customerId, some30DaysAgo);
            ...
            {
                var customerPersonalAgentFlightExpertRate =
                    CalculateAgentExpertRateBasedOnTimeRelevance(flightRatingsMatchCustomer, "Flight", agent.Name);
            ...
            {
                var agentPropertySelfExpertRate =
                    CalculateAgentExpertRateBasedOnTimeRelevance(propertiesRatingsMatchCustomer, "Property",
                        agent.Name);
            ...
            {
                var agentAttractionSelfExpertRate =
                    CalculateAgentExpertRateBasedOnTimeRelevance(attractionsRatingsMatchCustomer, "Attractions",
                        agent.Name);
    ...
    public Action SaveRecommendation(PreferencesResponse preferencesResponse, Guid clientRequestId)
    {
        ...
        var flightDepartureRecommendation = preferencesResponse.FlightRecommendationResponse!
            .FlightDirectionRecommendation!.DepartureFlightRecommendation!
            .ToEntity(flightDepartureRecommendationInitialAssignedAgentId);
        ...
        var flightReturnRecommendation = preferencesResponse.FlightRecommendationResponse!
            .FlightDirectionRecommendation!.ReturnFlightRecommendation!
            .ToEntity(flightReturnRecommendationInitialAssignedAgentId);
        ...
        var propertyRecommendationInitialAssignedAgentId = _context.Agents.SingleOrDefault(a =>
            a.Name == preferencesResponse.PropertyPreferencesResponse!.PropertyRecommendationBModel
                .InitialAssignedAgentName)!.Id;
        var propertyRecommendation =
            preferencesResponse.PropertyPreferencesResponse!.PropertyRecommendationBModel.ToEntity(
                propertyRecommendationInitialAssignedAgentId);
        ...
        var departureFlightConnections =
            preferencesResponse.FlightRecommendationResponse!.FlightDirectionRecommendation!
                .DepartureFlightRecommendation!.FlightConnection.Select(departureFlightConnection =>
                    departureFlightConnection
        ...
        var flightDirectionRecommendation =
            preferencesResponse.FlightRecommendationResponse.FlightDirectionRecommendation.ToEntity(
                flightDepartureRecommendation.Id, flightReturnRecommendation.Id);
        ...
        var recommendation = RecommendationMapper.ToEntity(flightDirectionRecommendation.Id, propertyRecommendation.Id,
            allAttractionsPackage.Id);
    ...
    public Task WritePreferencesResponse(PreferencesResponse preferencesResponse)
    {
        ...
        UserReportHelper.WritePreferencesResponse(preferencesResponse, departureFlightSourceAgentName,
            returnFlightSourceAgentName, propertySourceAgentName, attractionsSourceAgentName);

        return Task.CompletedTask;
    }

    public async Task<Task> CreateClientRequest(Guid customerId, Guid preferencesPackageId, Guid clientRequestId,
    ...
    private async Task<List<AgentServiceRating>?> GetAgentFlightRatingsMatchCustomer(Guid agentId, Guid customerId,
        DateTime searchDataTillDate)
    {
        var agentDepartureFlightRecommendationsForCurrentCustomer = _context.ClientRequests.Any(c =>
            ...
            (DateTime.Compare(c.RequestTimestamp, searchDataTillDate) > 0 ||
             DateTime.Compare(c.RequestTimestamp, searchDataTillDate) == 0))
            ? await _context.ClientRequests.Include(ce => ce.EvaluationNavigation)
                .ThenInclude(ef => ef.FlightEvaluation).ThenInclude(efd => efd.DepartureNavigation)
                .Where(c => c.Evaluation != null &&
                            c.CustomerId == customerId &&
                            c.Recommendation.FlightRecommendation.DepartureNavigation
                                .SourceAgentId == agentId &&
                            (DateTime.Compare(c.RequestTimestamp, searchDataTillDate) > 0 ||
                             DateTime.Compare(c.RequestTimestamp, searchDataTillDate) == 0)).Select(c =>
                    new AgentServiceRating
                    {
                        AgentId = agentId,
        ...
        var agentReturnFlightRecommendationsForCurrentCustomer = _context.ClientRequests.Any(c =>
            ...
            (DateTime.Compare(c.RequestTimestamp, searchDataTillDate) > 0 ||
             DateTime.Compare(c.RequestTimestamp, searchDataTillDate) == 0))
            ? await _context.ClientRequests.Include(ce => ce.EvaluationNavigation)
                .ThenInclude(ef => ef.FlightEvaluation).ThenInclude(efd => efd.ReturnNavigation)
                .Where(c => c.Evaluation != null &&
                            c.CustomerId == customerId &&
                            c.Recommendation.FlightRecommendation.ReturnNavigation
                                .SourceAgentId == agentId &&
        ...
        if (agentReturnFlightRecommendationsForCurrentCustomer != null)
            agentDepartureFlightRecommendationsForCurrentCustomer.AddRange(
                agentReturnFlightRecommendationsForCurrentCustomer);
        ...
        DateTime searchDataTillDate)
    {
        var agentPropertyRecommendationsForCurrentCustomer = _context.ClientRequests.Any(c =>
            ...
            (DateTime.Compare(c.RequestTimestamp, searchDataTillDate) > 0 ||
             DateTime.Compare(c.RequestTimestamp, searchDataTillDate) == 0))
            ? await _context.ClientRequests.Include(ce => ce.EvaluationNavigation)
                .ThenInclude(ep => ep.PropertyEvaluation)
                .Where(c => c.Evaluation != null &&
                            c.CustomerId == customerId &&
                            c.Recommendation.PropertyRecommendation
                                .SourceAgentId == agentId &&
        ...
        DateTime searchDataTillDate)
    {
        var agentAttractionPackagesRecommendationsForCurrentCustomer = _context.ClientRequests.Any(c =>
            ...
            (DateTime.Compare(c.RequestTimestamp, searchDataTillDate) > 0 ||
             DateTime.Compare(c.RequestTimestamp, searchDataTillDate) == 0))
            ? await _context.ClientRequests.Include(ce => ce.EvaluationNavigation)
                .ThenInclude(ea => ea.AttractionEvaluation)
                .Where(c => c.Evaluation != null &&
                            c.CustomerId == customerId &&
                            c.Recommendation.AttractionRecommendation
                                .SourceAgentId == agentId &&
        ...
        string serviceType, string agentName, bool isForSelfExpertRate = true) // added only for log
    {
        ...
        foreach (var agentServiceRating in agentServiceRatings)
        {
            ...
            var daysPassedTillTodaySinceEvaluation =
                (int) (DateTime.Today.Date - agentServiceRating.ServiceEvaluationDate.Date).TotalDays + 1;
            var evaluationBasedOnDate =
                CalculateEvaluationBasedOnDate(agentServiceRating.ServiceRating, daysPassedTillTodaySinceEvaluation);
    ...
    private async Task<Guid> ComposePreferencesPackageAsync(PreferencesRequest preferencesPayload)
    {
        ...
        if (preferencesPayload.CustomerFlightNavigation != null)
        {
            ...
            {
                var returnPeriodsPreference = preferencesPayload.CustomerFlightNavigation.ReturnNavigation
                    .DeparturePeriodPreference.ToEntity();
        ...
        if (preferencesPayload.CustomerPropertyNavigation is {PlaceTypeNavigation: { }})
        {
            ...
            {
                roomsAndBedsPreference =
                    preferencesPayload.CustomerPropertyNavigation.RoomsAndBedsNavigation.ToEntity();
        ...
        if (preferencesPayload.CustomerPropertyNavigation != null)
            if (amenitiesPreference != null && placeTypePreference != null && propertyTypePreference != null &&
                roomsAndBedsPreference != null)
            {
        ...
        if (preferencesPayload.CustomerFlightNavigation is
            {
                DepartureNavigation:
                {
        ...
        var departureCityId =
            _context.Cities.SingleOrDefault(c => c.Name == preferencesPayload.DepartureCityNavigation.Name)!.Id;
        var destinationCityId =
            _context.Cities.SingleOrDefault(c => c.Name == preferencesPayload.DestinationCityNavigation.Name)!.Id;
        ...
        var flightReturnEvaluations = _context.ClientRequests.Any(cr =>
            ...
            ? _context.ClientRequests.Include(e => e.EvaluationNavigation).ThenInclude(fe => fe.FlightEvaluation)
                ...
                .Where(cr => cr.Evaluation != null &&
                             cr.Recommendation.FlightRecommendation.ReturnNavigation.SourceAgentId.Equals(agentId) &&
                             (DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) > 0 ||
                              DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) == 0)).ToList().Select(c =>
                    new AgentServiceRating
                    {
                        AgentId = agentId,
                        ServiceRating = c.EvaluationNavigation.FlightEvaluation.ReturnNavigation.FlightRating,
    ...
    private async Task<List<AgentServiceRating>?> GetAgentPropertyRatings(Guid agentId, DateTime daysAgoFromToday)
    {
        return _context.ClientRequests.Any(cr =>
            ...
            (DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) > 0 ||
             DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) == 0))
            ? await _context.ClientRequests.Include(e => e.EvaluationNavigation)
                .ThenInclude(pe => pe.PropertyEvaluation)
                .Where(cr => cr.Evaluation != null &&
                             cr.Recommendation.PropertyRecommendation.SourceAgentId.Equals(agentId) &&
                             (DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) > 0 ||
                              DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) == 0))
    ...
    private async Task<List<AgentServiceRating>?> GetAgentAttractionsRatings(Guid agentId, DateTime daysAgoFromToday)
    {
        return _context.ClientRequests.Any(cr =>
            ...
            (DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) > 0 ||
             DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) == 0))
            ? await _context.ClientRequests.Include(e => e.EvaluationNavigation)
                .ThenInclude(ae => ae.AttractionEvaluation)
                .Where(cr => cr.Evaluation != null &&
                             cr.Recommendation.AttractionRecommendation.SourceAgentId.Equals(agentId) &&
                             (DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) > 0 ||
                              DateTime.Compare(cr.RequestTimestamp, daysAgoFromToday) == 0))
    ...
    private async Task UpdateTrustServiceEvaluationForAttractionsPackage(ServiceEvaluationDto serviceEvaluation)
    {
        ...
        {
            var trustedAgent = _context.TrustedAgents.Include(ta => ta.TrustedAgentRateNavigation)
                .ThenInclude(a => a.AttractionsTrustNavigation)
                .Single(a =>
            ...
            if (serviceEvaluation.AttractionEvaluation.FinalAttractionEvaluation >= 0.5)
            {
                ...
                trustedAgent.TrustedAgentRateNavigation.AttractionsTrustNavigation.LastPositiveEvaluation =
                    DateTime.Now;
            }
            else
            {
                trustedAgent.TrustedAgentRateNavigation.AttractionsTrustNavigation.NegativeEvaluation += 1;
                trustedAgent.TrustedAgentRateNavigation.AttractionsTrustNavigation.LastNegativeEvaluation =
                    DateTime.Now;
            }
    ...
    private async Task UpdateTrustServiceEvaluationForProperty(ServiceEvaluationDto serviceEvaluation)
    {
        ...
        {
            var trustedAgent = _context.TrustedAgents.Include(ta => ta.TrustedAgentRateNavigation)
                .ThenInclude(p => p.PropertyTrustNavigation)
                .Single(a =>
    ...
    private async Task UpdateTrustServiceEvaluationForReturnFlight(ServiceEvaluationDto serviceEvaluation)
    {
        var flightRecommendation =
            _context.ClientRequests.Include(r => r.Recommendation).ThenInclude(f => f.FlightRecommendation)
                .ThenInclude(fd => fd.ReturnNavigation)
                .Single(c => c.Id.Equals(serviceEvaluation.ClientRequestId)).Recommendation.FlightRecommendation
                .ReturnNavigation;
        ...
        {
            var trustedAgent = _context.TrustedAgents.Include(ta => ta.TrustedAgentRateNavigation)
                .ThenInclude(f => f.FlightTrustNavigation)
                .Single(a =>
    ...
    private async Task UpdateTrustServiceEvaluationForDepartureFlight(ServiceEvaluationDto serviceEvaluation)
    {
        var flightRecommendation =
            _context.ClientRequests.Include(r => r.Recommendation).ThenInclude(f => f.FlightRecommendation)
                .ThenInclude(fd => fd.DepartureNavigation)
                .Single(c => c.Id.Equals(serviceEvaluation.ClientRequestId)).Recommendation.FlightRecommendation
                .DepartureNavigation;
